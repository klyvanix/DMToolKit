...
namespace DMToolKit.Data
{
    public static class StaticStrings
    {
        ...
        {
            "This name generator operates under the philosophy that you can take two cool sounding affixations and smash them together to create new sounding names. A lot of the names generated with this tool won’t work, but you will be provided tools to catalogue and save those names that strike you as unique and cool.\n\nGenerating names means taking a prefix, IE the start of a word such as ‘Cal’ and a suffix, IE the end of a word such as ‘lan’. The resulting name will end up being Callan. If you like the name you will be able to tap on the name and save it for use later, or if the word sounds like it could be appropriate to function as a Prefix or Suffix you can add them to the list of affixations or name seeds.",
            "To Generate a name, you must first ensure that the prefix and suffix lists have been ‘seeded’. A basic list of prefixes and suffixes are provided standard the first time you open the tool, which will allow you to generate words or word parts out of the box. However, if you want to manually add names you can access those lists by tapping the Tools button at the bottom of the screen. \n\nIn the options page you will be able to manage the prefixes and suffixes of the names you generate on the first page. Notice the radio buttons at the bottom of the screen will operate other pages of options. By default, you will be on the Name seed options. The other options are for refining your generation of names.\r\n",
            "The Name Seed Options page is the default view you see as you come into the options page. Here there are sections for name seed management. To see a list of prefixes or suffixes you can tap on the list icon to the left of the text box in each section. There you can delete seeds if you don’t like them, and you are given the option to add seeds in the list pages.\n\nIf you wish to rapidly add seeds you can do so from the default view you can simply tap on the text box for the seed you wish to add and type in a seed. When you try to add it to the list if it does not disappear it means it is already in the list.\r\n",
            "In the Lock Prefix view, you will have the list of prefixes and a text box at the top of the screen. You can select a prefix you wish to lock as the selected prefix during name generation by tapping on a prefix in the list, or by typing one in the box at the top. If that prefix wasn’t in the list already it will be automatically added to the prefix list for use later.",
            "In the Lock Letter view you will be able to select the letter you wish to generate names for. Selecting ‘A’ will allow you to generate names that only start with the letter ‘A’. If no prefix exists with that letter in the list the name will randomly generate. If you wish to generate a name with the selected letter ensure that there are prefixes that begin with that letter in the list of prefixes.",
            "When you’re naming seeds are in a state you are happy with you can generate a single name by ensuring that the number at the bottom of the screen is 1 or more than 1 name by either tapping the ‘+’ and ‘-‘ on the counter at the bottom of the screen, or tapping the text box with the number to type the desired number of names to generate.\n\nAnything above 5 names will output only that number of names in the list when generating names. If your list of names gets too long be sure to manually clear the list using the ‘Clear’ button in the bottom left of the page.\r\n",
            "When you see a name you like, you may tap on the name in the list, and it will take you to a new name add page. At the top is the listed name and underneath is several buttons which you can add the name to. Tapping on those buttons will add it to the list and it will be unavailable to add again. If the name is already in one of the lists that list button for that corresponding list will be disabled."
        };
    }
    ...
    public static class CharacterAttributes
    {
        ...
        public static Dictionary<int, string> PositiveAttributeText = new Dictionary<int, string>()
        {
            ...
            {(int) PositiveAttribute.Ambitious, "Ambitiouss"},
